...
def bifpt(cs: np.ndarray, config: FptConfig, offset: int = 0, truncation: int = None):
    ...
    if config.power_threshold > 0:
        ...
        reverse = outside_shifted[[res.decay > 0 for res in outside_shifted]]
    # return inside
...
class FptPlus(Fpt):
    ...
    def qs(self):
        ...
        try:
            q = la.solve_toeplitz(
                (self.cs[K:2 * K], np.flip(self.cs[1:K + 1])), -self.cs[:K])
            return np.insert(np.flip(q), 0, 1)
        except la.LinAlgError as e:
            A = la.hankel(self.cs[1:K + 1], self.cs[K:2 * K])
            b = -self.cs[:K]
            qs_ = la.lstsq(A, b)[0]
            return np.insert(qs_, 0, 1)
    ...
    def zs(self):
        assert self.qs is not None
        # out_ = np.asfortranarray(np.zeros(degree), dtype=complex)
        # in_ = np.asfortranarray(self.qs, dtype=complex)
        # rootsg.cmplx_roots_gen(out_, in_, 0, 0, degree)
        # return out_

        # return hcroots.solve(self.qs)

        # degree = int(np.max(np.nonzero(self.qs)))
        # qs_ = self.qs[:degree+1]
        # _ctx = mpsolve.Context()
        # poly = mpsolve.MonomialPoly(_ctx, degree)
        # for n, q in enumerate(qs_):
        #     poly.set_coefficient(n, q)
        # out = _ctx.solve(poly)
        # return np.array(out)

        qs_ = np.asfortranarray(np.flip(self.qs), dtype=np.complex128)
        zs_ = np.zeros(len(self.qs)-1, dtype=np.complex128, order='F')
        rs_ = np.zeros(len(self.qs)-1, dtype=np.float64, order='F')
        i_ = np.array(0)
        eiscor_wrapper.z_poly_roots(qs_, zs_, rs_, i_)
    ...
    def ds(self):
        ...
        # i = np.arange(1, K + 1)
        # exponent = np.tile(i, (K, 1))
        # Pz = np.power(self.zs.reshape(-1, 1), exponent) @ self.ps[1:].reshape(-1, 1)
        # Qz = np.power(self.zs.reshape(-1, 1), exponent - 1) @ (self.qs[1:] * i).reshape(-1, 1)
        # d = np.divide(Pz, Qz)
        # return d.reshape((-1,)) / self.zs
